...
function anySvg(size, { bg, stroke = null, scale = 10.4 }) {
	const border = stroke
		? `<rect width="${size}" height="${size}" fill="${bg}" stroke="${stroke}" stroke-width="${Math.max(1, size / 180)}" />`
...
await writePng(join(staticDir, 'apple-touch-icon.png'), anySvg(180, dark), 180);
await writePng(join(staticDir, 'apple-touch-icon-light.png'), anySvg(180, light), 180);
await writePng(join(staticDir, 'icon-192.png'), anySvg(192, dark), 192);
await writePng(join(staticDir, 'icon-192-light.png'), anySvg(192, light), 192);
await writePng(join(staticDir, 'icon-512.png'), anySvg(512, dark), 512);
await writePng(join(staticDir, 'icon-512-light.png'), anySvg(512, light), 512);
await writePng(join(staticDir, 'icon-maskable-512.png'), maskableSvg(512, dark.bg), 512);
await writePng(join(staticDir, 'icon-maskable-512-light.png'), maskableSvg(512, light.bg), 512);
...
// Adaptive SVG: follows phone OS color scheme (home screen / tab favicon).
const adaptive = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" fill="none">
	<style>
		.bg { fill: #0B0B0C; }
		.frame { stroke: transparent; stroke-width: 0.75; }
		@media (prefers-color-scheme: light) {
			.bg { fill: #F7F9FB; }
			.frame { stroke: #B8C4D4; }
		}
	</style>
	<rect class="bg frame" width="32" height="32" rx="8" />
	<path d="M9 12.5V26" stroke="#8B5CF6" stroke-width="2.65" stroke-linecap="round" />
	<path d="M9 6.75V10.25" stroke="#8B5CF6" stroke-width="2.65" stroke-linecap="round" />
	<path
		d="M9 7h11.25a5.35 5.35 0 0 1 0 10.7H9"
