...
PHRASES: list[tuple[str, str]] = [
	...
	("cross body hammer curl", "молотковый подъём на бицепс крест-накрест"),
	("cross-body hammer curl", "молотковый подъём на бицепс крест-накрест"),
	("bridge - mountain climber (cross body)", "мост - скалолаз с касанием локтя к колену"),
	("mountain climber (cross body)", "скалолаз с касанием локтя к колену"),
	("clean-grip front squat", "приседания на груди хватом для взятия"),
	("clean grip front squat", "приседания на груди хватом для взятия"),
	("pronate-grip triceps extension", "разгибание на трицепс пронированным хватом"),
	("pronate grip triceps extension", "разгибание на трицепс пронированным хватом"),
	("reverse-grip press", "жим обратным хватом"),
	("reverse grip press", "жим обратным хватом"),
	("incline reverse-grip press", "жим обратным хватом на наклонной скамье"),
	("front chest squat", "приседания на груди"),
	("pullover to press", "пуловер с жимом"),
	("side bent", "боковые наклоны"),
	("astride jumps", "прыжки ноги врозь"),
	("half sit-up", "неполные скручивания"),
	("half knee bends", "полуприседания"),
	("modified hindu push-up", "облегчённые отжимания «индиец»"),
	("modified push up to lower arms", "облегчённые отжимания с опорой на предплечья"),
	("band fixed back close grip pulldown", "вертикальная тяга узким хватом с резинкой"),
	("standing cross-over high reverse fly", "обратное разведение в верхнем кроссовере стоя"),
	("bent arm pullover", "пуловер согнутыми руками"),
	("decline bent arm pullover", "пуловер согнутыми руками на наклонной скамье вниз"),
	("decline wide-grip pullover", "пуловер широким хватом на наклонной скамье вниз"),
...
def polish_ru_title(result: str) -> str:
	...
	# «Одной рукой …» → movement first (same idea as grip).
	result = re.sub(
		r"^(одной рукой|одной ногой|согнутой рукой|двумя ногами)\s+(.+)$",
		lambda m: f"{m.group(2)} {m.group(1).lower()}",
		result,
		flags=re.IGNORECASE,
	)
	...
	# Decline-first salads → movement + «на наклонной скамье вниз» / «головой вниз».
	result = re.sub(r"^вниз головой отжимания\b", "отжимания головой вниз", result, flags=re.I)
	result = re.sub(r"^вниз головой скручивания\b", "скручивания на наклонной скамье вниз", result, flags=re.I)
	result = re.sub(
		r"^вниз головой (.+)$",
		lambda m: f"{m.group(1)} на наклонной скамье вниз",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"\bна наклонной скамье вниз на наклонной скамье вниз\b",
		"на наклонной скамье вниз",
		result,
		flags=re.I,
	)
	# Incline adjective-first lifts (not already «на наклонной скамье»).
	result = re.sub(
		r"^наклонный внутренний (подъём|подъёмы) на бицепс\b",
		r"внутренний \1 на бицепс на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"^наклонный (подъём|подъёмы) на бицепс\b",
		r"\1 на бицепс на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"^наклонный (подъём|подъёмы) плеча\b",
		r"\1 плеч на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"^наклонный (жим) узким хватом\b",
		r"\1 узким хватом на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"^наклонный (жим) от груди\b",
		r"\1 от груди на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(
		r"^наклонный (жим|подъём|подъёмы|разведение|скручивание|жим к низу)\b",
		r"\1 на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(r"\bна наклонной скамье на наклонной скамье\b", "на наклонной скамье", result, flags=re.I)
	# Prone / awkward starts.
	result = re.sub(
		r"^на животе наклонный (молотковый )?подъём на бицепс лёжа\b",
		lambda m: f"подъём на бицепс лёжа на животе на наклонной скамье"
		if not m.group(1)
		else "молотковый подъём на бицепс лёжа на животе на наклонной скамье",
		result,
		flags=re.I,
	)
	result = re.sub(r"^на животе скручивание с фитболом лёжа\b", "скручивание лёжа на животе на фитболе", result, flags=re.I)
	result = re.sub(r"^на животе бицепса бедра лёжа с поддержкой\b", "сгибание бицепса бедра лёжа на животе с поддержкой", result, flags=re.I)
	result = re.sub(r"^прямой тяга высокая спины\b", "горизонтальная тяга прямой спины", result, flags=re.I)
	result = re.sub(r"^обратный вниз головой\b", "обратный", result, flags=re.I)
	result = re.sub(r"\bв машине Смита\b", "в тренажёре Смита", result, flags=re.I)
	result = re.sub(r"^обратный подъёмы на носки\b", "обратные подъёмы на носки", result, flags=re.I)
	result = re.sub(r"^наклонный подъёмы плеча\b", "подъёмы плеч на наклонной скамье", result, flags=re.I)
	result = re.sub(r"скручивание разведение\b", "разведение с поворотом", result, flags=re.I)

	# Re-apply grip / one-arm order after decline/incline rewrites may put them first again.
	result = re.sub(
		r"^(узким|узкий|широким|широкий|обратным|обратный|молотковым|молотковый|прямым|прямой|нейтральным|нейтральный) хватом (.+)$",
		lambda m: f"{m.group(2)} {m.group(1).lower().replace('узкий','узким').replace('широкий','широким').replace('обратный','обратным').replace('молотковый','молотковым').replace('прямой','прямым').replace('нейтральный','нейтральным')} хватом",
		result,
	...
	# Inline «вариант N» → trailing (вариант N); dedupe existing paren form.
	ver = re.search(r"\bвариант\s*(\d+)\b", result, flags=re.I)
	if ver:
		n = ver.group(1)
		result = re.sub(r"\s*\(\s*вариант\s*\d+\s*\)", "", result, flags=re.I)
		result = re.sub(r"\s*вариант\s*\d+\b", "", result, flags=re.I)
		result = normalize_spaces(result)
		result = f"{result} (вариант {n})"
	result = normalize_spaces(result)
...
GYM_STANDARD_OVERRIDES: dict[str, str] = {
	...
	"0039": "Фронтальные приседания со штангой",
	"0042": "Приседания со штангой на груди",
	"0282": "Подъёмы корпуса на наклонной скамье вниз",
	"0316": "Разведение гантелей на наклонной скамье (другой ракурс)",
	"0674": "Подтягивания обратным хватом (другой ракурс)",
	"1326": "Подтягивания обратным хватом",
	"0009": "Отжимания на брусьях на грудь с поддержкой (на коленях)",
	"0019": "Отжимания на брусьях на трицепс с поддержкой (на коленях)",
	"0426": "Жим гантелей стоя над головой",
	"0029": "Приседания на груди хватом для взятия",
	"0048": "Жим обратным хватом на наклонной скамье",
	"0758": "Жим обратным хватом на наклонной скамье в тренажёре Смита",
	"0764": "Жим обратным хватом в тренажёре Смита",
	"0373": "Разгибание на трицепс пронированным хватом с гантелями",
	"0022": "Пуловер с жимом со штангой",
	"0225": "Обратное разведение в верхнем кроссовере стоя",
	"1421": "Облегчённые отжимания с опорой на предплечья",
	"3217": "Облегчённые отжимания «индиец»",
	"3202": "Неполные скручивания",
	"3221": "Полуприседания",
	"3220": "Прыжки ноги врозь",
	"0096": "Боковые наклоны со штангой (вариант 2)",
	"3117": "Вертикальная тяга узким хватом с резинкой",
	"0208": "Горизонтальная тяга обратным хватом сидя на блоке",
	"1316": "Пуловер согнутыми руками со штангой",
	"0034": "Пуловер согнутыми руками на наклонной скамье вниз",
	"0037": "Пуловер широким хватом на наклонной скамье вниз",
	"1657": "Молотковый подъём на бицепс крест-накрест (вариант 2)",
	"0193": "Горизонтальная тяга одной рукой на блоке (на коленях)",
	"0016": "Сгибание бицепса бедра лёжа на животе с поддержкой",
	"0279": "Отжимания головой вниз",
	"0747": "Жим из-за головы в тренажёре Смита",
	"0748": "Жим лёжа в тренажёре Смита",
	"0752": "Становая тяга в тренажёре Смита",
	"0761": "Тяга узким хватом в тренажёре Смита",
	"0768": "Болгарские выпады в тренажёре Смита",
	"0769": "Спринтовый выпад в тренажёре Смита",
	"1394": "Обратные подъёмы на носки в тренажёре Смита (другой ракурс)",
	"1719": "Жим узким хватом лёжа на наклонной скамье",
	"0167": "Высокая тяга на блоке (на коленях)",
	"0213": "Высокая тяга сидя на блоке (V-гриф)",
	"0548": "Высокая тяга сумо с гирей",
	"0581": "Высокая тяга в рычажном тренажёре",
	...
	"1490": "Подъём на носки стоя (по лестнице)",
	"1369": "Подъём на носки с резинкой двумя ногами (другой ракурс)",
	"1370": "Подъём на носки со штангой на полу",
	"1382": "Подъём на носки на фитболе у стены",
	"3240": "Подъём на носки на фитболе у стены (мяч между коленей)",
	"3241": "Подъём на носки на фитболе у стены (мяч между лодыжек)",
	...
	"0079": "Обратное сгибание запястий со штангой (вариант 2)",
	...
	"0767": "Шраги в тренажёре Смита (другой ракурс)",
	"2800": "Сидя поочерёдный подъём ног со штангой (жен.)",
	"2799": "Сидя поочерёдный подъём ног со штангой",
	"0242": "Обратное скручивание с подтягиванием коленей на блоке",
	"0873": "Обратное скручивание на блоке",
	"1377": "Растяжка икр руками у стены",
	"1407": "Растяжка икр толчком руками у стены",
	"0432": "Тяга на прямых ногах с гантелями",
	"0434": "Становая тяга на прямых ногах с гантелями",
	"3296": "Передний рычаг",
	"3295": "Передний рычаг (повторы)",
	"0499": "Горизонтальные подтягивания",
	"0498": "Горизонтальные подтягивания с лямками",
	"0537": "Взятие и толчок с гирей одной рукой",
	"0538": "Толчок с гирей одной рукой",
	"2801": "Подъём ноги с поворотом (жен.)",
	"2802": "Подъём ноги с поворотом",
}
