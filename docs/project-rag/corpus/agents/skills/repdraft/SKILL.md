<!-- source: .agents/skills/repdraft/SKILL.md -->
<!-- synced: 2026-09-23 -->

---
name: repdraft
description: >-
  Fitness PWA (SvelteKit 5 + Supabase + Playwright). Use when working in
  repdraft: sacred loop UX polish, domain→storage→stores→routes,
  catalog/workout flows, Supabase auth, PWA, Vercel deploy.
---
...
- Снова добавлять store shells (Capacitor и т.п.) без явного запроса

## MCP / env

Project MCP: `.cursor/mcp.json` — Playwright + Supabase (read-only, project-scoped).

Env (см. `.env.example`): `PUBLIC_SUPABASE_URL`, `PUBLIC_SUPABASE_ANON_KEY`, `PUBLIC_SITE_URL`, `PUBLIC_PRIVACY_*` (required before public), optional `PUBLIC_WEB_ORIGIN` / Turnstile. Server-only: `SUPABASE_SERVICE_ROLE_KEY` для delete account.
